...
export default ({ logTypeSkipped, logTypeFocused } = {}) => {
  const hasJsSkippedTests = filename =>
    fileAddedLineMatch(
      filename,
      /xdescribe|describe\.skip|xit|it\.skip|test\.skip/,
    );

  const hasJsFocusedTests = filename =>
    fileAddedLineMatch(
      filename,
      /fdescribe|describe\.only|fit|it\.only|test\.only/,
    );

  const logSkipped = getMessageLogger(logTypeSkipped);
  const logFocused = getMessageLogger(logTypeFocused);
  const jsFiles = committedFilesGrep(/(\.test\.js|\.test\.jsx|\.spec\.js)$/);
